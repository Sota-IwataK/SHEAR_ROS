...
class AmirTrajectoryNode(Node):
    ...
    def joint_states_cb(self, msg):
        for i, name in enumerate(msg.name):
            if name == 'Joint_2': self.theta0 = msg.position[i]
            if name == 'Joint_3': self.theta1 = msg.position[i]
            if name == 'Joint_4': self.theta2 = msg.position[i]
    ...
    def control_loop(self):
        ...
        # Amirの5DOF関節角に変換
        print(f'J1_raw={DegToRad(169/2) - Theta0(pose_holo[1], pose_holo[0]):.3f} J3_raw={-DegToRad(146) - self.theta1:.3f} J5_raw={DegToRad(167.5):.3f} pose={pose_holo}')
        # Joint_1：ベース旋回
        ...
        # Joint_2：固定オフセット（YouBotのDegToRad(135)相当）
        # Amirの関節構造に合わせて調整が必要
        joint_2 = float(np.clip(
            DegToRad(135) - self.theta0,
